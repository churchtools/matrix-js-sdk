...
MegolmEncryption.prototype._ensureOutboundSession = async function (devicesInRoom, blocked) {
  ...
  const prepareSession = async oldSession => {
    ...
    await Promise.all([(async () => {
      ...
      await this._shareKeyWithDevices(session, key, payload, devicesWithoutSession, errorDevices, 2000);
      ...
      (async () => {
        ...
        for (const {
          ...
          const deviceId = deviceInfo.deviceId; // mark this device as "handled" because we don't want to try
...
MegolmEncryption.prototype._shareKeyWithDevices = async function (session, key, payload, devicesByUser, errorDevices, otkTimeout) {
  const devicemap = await olmlib.ensureOlmSessionsForDevices(this._olmDevice, this._baseApis, devicesByUser, otkTimeout);
